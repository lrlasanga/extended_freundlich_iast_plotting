...
if __name__ == "__main__":
    ...
    for idx, CA in enumerate(Ce_A):
        ...
        plt.plot(log_Ce_B, log_qe_B, linestyle='-', label=f'{args.B} = {CA} ppm, {args.C} = {Ce_C[idx]} ppm')
    plt.xlabel('log(Ce)')
    plt.ylabel('log(qe)')
    plt.title(f'Plots of {args.A} adsorption against {args.B} and {args.C} adsorption')
    plt.grid(True)
    plt.legend()
    plt.show()
    ...
    for idx, CB in enumerate(Ce_B):
        ...
        plt.plot(log_Ce_A, log_qe_A, linestyle='-', label=f'{args.A} = {CB} ppm, {args.C} = {Ce_C[idx]} ppm')
    plt.xlabel('log(Ce)')
    plt.ylabel('log(qe)')
    plt.title(f'Plots of {args.B} adsorption against {args.A} and {args.C} adsorption')
    plt.grid(True)
    plt.legend()
    plt.show()
    ...
    for idx, CC in enumerate(Ce_C):
        ...
        plt.plot(log_Ce_C, log_qe_C, linestyle='-', label=f'{args.B} = {CC} ppm, {args.A} = {Ce_A[idx]} ppm')
    plt.xlabel('log(Ce)')
    plt.ylabel('log(qe)')
    plt.title(f'Plots of {args.C} adsorption against {args.A} and {args.B} adsorption')
    plt.grid(True)
    plt.legend()
    plt.show()
